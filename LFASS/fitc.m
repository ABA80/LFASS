...
if length(ExcelData) > right_interval_min
    absmin = find(ExcelData == min(ExcelData(left_interval_min:right_interval_min)));   %min before= 77 for 20140926_BF_6autoph_HS42C_d1-14
    else
...
if plot_original == 'y'
    ...
    if ~isempty(tag)
        title(strcat('Smoothed death fluorescence curve for ', tag{length(tag)}));
        else
        title(strcat('Smoothed blue fluorescence curve ', int2str(num_line)));
    end
    xlabel('Time (minutes)')
...
if strcmp(testfit(time_length_bis, ExcelData_bis, @(b,x)modelfunc( b(1),b(2),b(3),b(4),b(5),x ), [1;0.5;50;0.05;0.3], opts),'true') == 1
    ...
    if plotting == 'y'
        ...
        if ~isempty(tag)
            title(strcat('Fitted death fluorescence curve for', tag{length(tag)}));
            else
            title(strcat('Fitted blue fluorescence curve', int2str(num_line)));
        end
        xlabel('time in minuts')
        ylabel('Normalised smoothed death fluorescence intensity')
        hold on
    ...
    if plotting == 'y'
        ...
        if ~isempty(tag)
            ...
            title(strcat('Smoothed death fluorescence curve  ', int2str(num_line)));
        end
        xlabel('Time (minutes)')
        ylabel('Normalized smoothed death fluorescence intensity ')
        hold on
